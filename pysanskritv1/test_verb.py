...
def v_file_init_alt1_pre(filein,fileout,tensecode,voicecode=None):
 nin = 0
 nout = 0
 f = codecs.open(filein,"r","utf-8")
 fout = codecs.open(fileout,"w","utf-8")
 ...
 for line in f:
  ...
  if line == '':
   continue
  # Example. line = aMS:10P
  m = re.search(r'^(.*?):([0-9]+)([AP]?)$',line)
  if not m:
   print "PROBLEM WITH INPUT LINE:",line
   exit(1)
  (root,theclass,pada) = (m.group(1),m.group(2),m.group(3))
  if (theclass == '0') or (pada == ''):
   print "skipping ",line
   continue
  dbg=False
  dtype=None
...
if __name__ == "__main__":
 filein = sys.argv[1]
 fileout = sys.argv[2]
 tensecode = sys.argv[3]
 if tensecode in ['pre']:
  v_file_init_alt1_pre(filein,fileout,tensecode)
 exit(1)
 if tensecode == 'fut':
