"""huet_parts.py
   ...
   This program is passive past participle
"""
...
def main(filein,hpartcode,fileout,fileout1):
 """ Use lxml """
 recs=[]
 ...
 for _, element in etree.iterparse(filein, tag='f'):
  ...
  for  inflected_form in inflected_forms:
   ...
   assert (theclass == None),"Unexpected theclass=%s for stem=%s"%(theclass,stem)
   theclass = ''  # so rest of code works without change.
   ...
   if stem not in stemdict:
    ...
    stemlist.append(stem)
   rec = stemdict[stem]
   # to minimize updates, take theclass and pada as empty strings
   theclass = ''
   pada = ''
