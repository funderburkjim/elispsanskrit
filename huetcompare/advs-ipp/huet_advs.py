"""huet_advs.py
   ...
   This program is indeclineable  past participle (absolutive -tvA)
"""
...
def main(filein,hadvcode,fileout):
 ...
 for _, element in etree.iterparse(filein, tag='f'):
  ...
  for  inflected_form in inflected_forms:
   ...
   if tag != hadvcode:
    continue
   [cjelt] = list(inflected_form) # nominal, kridanta
...
if __name__ == "__main__":
 filein = sys.argv[1]
 hadvcode = sys.argv[2]
 assert (hadvcode in ["ab"]),"Unexpected adverb abbreviation=%s\n" % hadvcode
 hadv_to_pysan={"ab":"ipp"}
