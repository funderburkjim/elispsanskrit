"""verbs-prim-pas.py
   ...
   Handle only the 'v' forms where cj is 'prim' and 'sys' is 'pas'
...
def main(filein,fileout,fileout1):
 ...
 for _, element in etree.iterparse(filein, tag='f'):
  ...
  for  inflected_form in inflected_forms:
   ...
   if stem not in stemdict:
    ...
    stemlist.append(stem)
   rec = stemdict[stem]
   rec.update_form(form,tense,person,number)
 ...
 for stem in stemlist:
  ...
  tabkeys = rec.tables.keys() 
  tabkeys.sort()
  for tense in tabkeys:
   t = tabs[tense]
   tstr = prettify(t)
   cp = 'PASSIVE' # passive voice
