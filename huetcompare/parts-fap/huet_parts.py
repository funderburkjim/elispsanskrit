"""huet_parts.py
   ...
   This program is specialized for future active participles,
...
class Huet_part(object):
 def __init__(self,stem):
  self.stem=stem
  self.cpms = [] # class-pada-gender (class is None, translated to empty string
...
def main(filein,hpartcode,hpada,fileout,fileout1):
 ...
 for _, element in etree.iterparse(filein, tag='f'):
  ...
  for  inflected_form in inflected_forms:
   ...
   if pada != hpada:
    continue
   theclass = partelt.get("gn") # class of underlying root. Always None
   assert (theclass == None),"Unexpected class=%s for stem=%s"%(theclass,stem)
   # Change theclass to empty string, to minimize other program changes
   theclass = ''
...
if __name__ == "__main__":
 ...
 if [hpartcode,hpada] == ['pfut','para']:
  filesfx = 'fap'
 elif [hpartcode,hpada] == ['pfut','atma']:
  filesfx = 'fmp'
 else:
