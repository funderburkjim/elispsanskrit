"""huet_parts.py
   ...
   08-18-2016
   This program is specialized for passive future participle (huet pfutp)
"""
...
def main(filein,hpartcode,hpada,fileout,fileout1):
 ...
 for _, element in etree.iterparse(filein, tag='f'):
  ...
  for  inflected_form in inflected_forms:
   ...
   # SL_parts.xml shows a 'gn' attribute, with values 1,2 or 3
   # We'll treat this as a 'class'
   theclass= partelt.get("gn")
   assert (theclass in ["1","2","3"]),"Unknown gana =%s for stem=%s\n" %(theclass,stem)
   # theclass serves no useful function in the comparison. Thus, set it to ''
   # for further processing
   theclass=''
   # pada is not used. Treat it as empty string
   pada = ''
   if stem not in stemdict:
...
if __name__ == "__main__":
 ...
 if [hpartcode,hpada] == ['ppr','para']:
  ...
  filesfx = 'prpp'
 elif [hpartcode,hpada] == ['pfutp',None]:
  filesfx = 'potp'
 else:
