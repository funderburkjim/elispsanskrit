""" pysan_stems.py
    Aug 19, 2016
    for future passive participles (gerundives)
"""
...
def stem_to_kind(stem):
 # identify 'type' of each value, based on ending
 if stem.endswith('tavya'):
  g=3
 elif stem.endswith('Iya'):
  g=2
 else:
  g=1
 return "%s"%g
...
def main(filein,fileout):
 ...
 for stem in stemlist:
  rec = stemdict[stem]
  rcps = rec.rcps
  # for the main output, remove the root, class, and pada
  """
  ...
  cpstr = ','.join(cpstrs)
  """
  #cpstr=stem_to_kind(stem)
  cpstr=''
  out = "%s:%s" % (stem,cpstr)
  fout.write(out + "\n")
 fout.close()
if __name__ == "__main__":
 filein = sys.argv[1] #MW-verb-potp.txt
 sfx = sys.argv[2]
