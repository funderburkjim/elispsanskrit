"""verbs_tp.py
   ...
   08-15-2016.  Modified to get pef (periphrastic future), even though
   this is not a 'tense paradigm'
"""
...
class Huet_verb_prim_tp(object):
 ...
 def update_form(self,form,pada,person,number):
  if pada not in self.padas:
   self.padas.append(pada)
   self.tables[pada] = [[] for i in xrange(0,9)]
  t = self.tables[pada]
...
def main(filein,htense,fileout,fileout1):
 ...
 for _, element in etree.iterparse(filein, tag='f'):
  ...
  for  inflected_form in inflected_forms:
   ...
   if stem not in stemdict:
    ...
    stemlist.append(stem)
   rec = stemdict[stem]
   rec.update_form(form,pada,person,number)
 ...
 for stem in stemlist:
  ...
  for pada in tabkeys:
   t = tabs[pada]
   tstr = prettify(t)
   out = "%s %s %s:%s" %(stem,htense,pada,tstr)
   fout.write(out + "\n")
 fout.close()
...
if __name__ == "__main__":
 filein = sys.argv[1]
 htense = sys.argv[2];
 assert (htense in ["fut","prf","aor","inj","cnd","ben","pef"]),"Unexpected tense abbreviation=%s\n" % htense
